/**
 * This script fixes issues with the Plugin model migration.
 * It can be used if the migration fails but the table already exists in the database.
 * 
...
SELECT * FROM _prisma_migrations WHERE migration_name = '20250521050000_add_plugin_model';
...
SET applied = 1,
    rolled_back = 0,
    rolled_back_at = NULL
WHERE migration_name = '20250521050000_add_plugin_model';
...
SELECT
  ...
  NOW(),
  '20250521050000_add_plugin_model',
  'Migration manually marked as applied',
  NULL,
  NOW(),
  1
WHERE NOT EXISTS (
  SELECT 1 FROM _prisma_migrations WHERE migration_name = '20250521050000_add_plugin_model'
);
*/

console.log(`
To fix the Plugin model migration issue, execute the following SQL commands
