        ...
        <div className="flex items-centre justify-between">
          ...
          <div className="flex items-centre space-x-2">
            ...
            <span className="px-2 py-1 text-xs font-semibold rounded-full bg-blue-100 text-blue-800">
              {scorePercentage}% ({currentTotalScore}/{totalPoints})
            </span>
          </div>
        </div>
        <div className="mt-2 text-grey-600">
          <p>Student: {response.user.name}</p>
          ...
          <Card>
            ...
            <CardContent>
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                <div>
                  <h3 className="text-sm font-medium text-grey-500">Title</h3>
                  <p className="text-grey-900">{assessment.title}</p>
                </div>
                <div>
                  <h3 className="text-sm font-medium text-grey-500">Subject</h3>
                  <p className="text-grey-900 capitalize">{assessment.subject}</p>
                </div>
                <div>
                  <h3 className="text-sm font-medium text-grey-500">Key Stage</h3>
                  <p className="text-grey-900 capitalize">
                ...
                <div>
                  <h3 className="text-sm font-medium text-grey-500">Type</h3>
                  <p className="text-grey-900 capitalize">{assessment.type}</p>
                </div>
                <div>
                  <h3 className="text-sm font-medium text-grey-500">Passing Score</h3>
                  <p className="text-grey-900">{assessment.passingScore}%</p>
                </div>
                <div>
                  <h3 className="text-sm font-medium text-grey-500">Time Taken</h3>
                  <p className="text-grey-900">
                    {response.startedAt && response.completedAt ? (
                      `${Math.round((new Date(response.completedAt).getTime() - new Date(response.startedAt).getTime()) / 60000)} minutes`
                    ) : 'Unknown'}
                  </p>
                </div>
              </div>
          ...
          <Card>
            ...
            <CardContent>
              <div className="space-y-4">
                <div className="flex items-centre justify-between">
                  <span className="text-grey-700">Total Score</span>
                  <span className="font-medium">{currentTotalScore}/{totalPoints} ({scorePercentage}%)</span>
                </div>
                <div className="w-full bg-grey-200 rounded-full h-2.5">
                  <div 
                    className={`h-2.5 rounded-full ${isPassing ? 'bg-green-600' : 'bg-red-600'}`} 
                    style={{ width: `${scorePercentage}%` }}
                  ></div>
                </div>
                
                <h3 className="text-lg font-medium mt-6 mb-2">Questions</h3>
                <div className="space-y-2">
                  {assessment.questions.map((question, index) => {
                    const answer = response.answers.find(a => a.questionId === question.id);
                    const grade = grades[question.id];
                    const scorePercent = grade ? Math.round((grade.score / question.points) * 100) : 0;
                    
                    return (
                      <div key={question.id} className="flex items-centre justify-between p-3 border rounded-md">
                        <div className="flex-1">
                          <p className="font-medium">Question {index + 1}</p>
                          <p className="text-sm text-grey-500 truncate">{question.content.substring(0, 60)}...</p>
                        </div>
                        <div className="flex items-centre space-x-4">
                          <div className="text-right">
                            <p className="text-sm font-medium">{grade?.score || 0}/{question.points}</p>
                            <p className="text-xs text-grey-500">{scorePercent}%</p>
                          </div>
                          <div className="w-16 bg-grey-200 rounded-full h-2">
                            <div 
                              className={`h-2 rounded-full ${
                                scorePercent >= 70 ? 'bg-green-600' : 
                                scorePercent >= 40 ? 'bg-yellow-500' : 'bg-red-600'
                              }`} 
      ...
      {activeTab === 'grade' && (
        <div className="space-y-6">
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
            <Card>
              <CardHeader>
                <CardTitle>Question & Student Answer</CardTitle>
              </CardHeader>
              <CardContent>
                {renderQuestionContent(currentQuestion)}
                <div className="mt-6">
                  <h3 className="text-lg font-medium mb-2">Student Answer</h3>
                  ...
                  <div>
                    ...
                    <div className="flex items-centre space-x-2">
                      <input
                        ...
                        onChange={(e) => handleScoreChange(currentQuestion.id, parseInt(e.target.value))}
                        className="w-20 px-3 py-2 border border-grey-300 rounded-md focus:outline-none focus:ring-2 focus:ring-blue-500"
                      />
                      <span className="text-grey-500">/ {currentQuestion.points}</span>
                    </div>
                  </div>
                    ...
                    <div className="mt-4">
                      ...
                      {aiSuggestions[currentQuestion.id] && (
                        <div className="mt-4 p-4 border rounded-md bg-blue-50">
                          <h4 className="font-medium text-blue-800 mb-2">AI Suggested Grading</h4>
                          <p className="text-sm mb-1">
                            <span className="font-medium">Score:</span> {aiSuggestions[currentQuestion.id].score}/{currentQuestion.points}
                          </p>
                          <p className="text-sm mb-3">
                            <span className="font-medium">Feedback:</span> {aiSuggestions[currentQuestion.id].feedback}
                          </p>
                          <Button
                            size="sm"
